...
def process_pdf(file_path):
    try:
        ...
        print(f"Parsed {len(nodes)} nodes from the PDF.")
        
        # Create vector index and retriever
        index = VectorStoreIndex(nodes)
        retriever = index.as_retriever(similarity_top_k=3)
        print("Index created and retriever initialized.")
        
        # Define tools with proper pydantic schemas
        tools = [
            FunctionTool.from_defaults(
                name="invoice_retriever",
                fn=lambda query: [{"text": node.text} for node in retriever.retrieve(query)],
...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        ...
        try:
            ...
            print("PDF processed and agent created.")
            
            # Extract invoice details
            invoice_details = agent.query("Extract all important invoice details including invoice number, date, amounts, vendor, and line items")
            print("Invoice details extracted.")
